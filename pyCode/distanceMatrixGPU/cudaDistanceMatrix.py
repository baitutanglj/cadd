...
def numba_func(x,y):
    ...
    x_norm = cDataStructs.TanimotoSimilarity(x,y)
    return x_norm
...
class DistanceMatrix:
    ...
    def _cuda_norm_square(self, X,axis=1):
        """Caluclate L2-norm_square on gpu.
        Parameters
        ----------
        X: array
            Array to normalize
        ...
        normX: array
            Normalized array
        """
        X = misc.sum(X ** 2, axis=axis, keepdims=True)
        one_matrix = misc.ones((max(X.shape),max(X.shape)),dtype=X.dtype)
        X = misc.multiply(X, one_matrix)
        return X

    def _get_dist(self,X):
        one_matrix = misc.ones_like(X)
        X_dist = one_matrix - X
        return X_dist
    ...
    def _get_XTX_cuda_Tanimoto(self, X, x_1, x_2, y_1, y_2):
        """Caluclate dot product between two array on gpu.
        Parameters
        ----------
        X: array
        ...
        """
        '''TanimotoSimilarity'''
        X_f = np.ascontiguousarray(X[x_1:x_2, :])
        X_b = np.ascontiguousarray(X[y_1:y_2, :])
        X_f, X_b = gpuarray.to_gpu(X_f), gpuarray.to_gpu(X_b)
        pq = linalg.dot(X_f, X_b,transb="T")
        p_square = self._cuda_norm_square(X_f,axis=1)
        q_square = self._cuda_norm_square(X_b.T,axis=0)
        X = (pq/(p_square+q_square-pq))
        X_dist = self._get_dist(X).get()
        return X_dist
    ...
    def calculate_distmatrix(self, X, nr_square=4):
        ...
        if np.float32 != X.dtype:
            print("Warning: Array is not float32. The array will be convered from ", X.dtype,
                  " to float32 for speed up.")
            X = X.astype(np.float32)
        l = X.shape[0] / nr_square
        assert l >= 2, "Please pick fewer number of sub-arrays each has a length longer than 2 elements. Currently " + str(
            round(l, 2))
        if self.float == "float64":
            assert X.shape[
                       0] >= 10000, "If using float64 ensure that that X.shape[0] > 10 000. Otherwise use float32 or cpu-version."
            ...
            for j in range(self.nr_square):
                ...
                if j <= i:
                    ...
                    if self.gpu:
                        ...
                        XTX = self._get_XTX_cuda_Tanimoto(X, *[x_1, x_2, y_1, y_2])
                    else:
                    ...
                    self.file_dist.create_dataset(
                        str(x_1) + "-" + str(x_2 - 1) + "_" + str(y_1) + "-" + str(y_2 - 1) + ':subArray_' + str(
                            i) + "_" + str(j) + "_" + str(arr_nr), data=subdistMatrix_flatten,
                        dtype=subdistMatrix_flatten.dtype)
                    arr_nr += 1
