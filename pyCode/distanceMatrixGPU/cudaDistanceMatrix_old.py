...
class DistanceMatrix:
    ...
    def _get_XTX_cuda(self, X, x_1, x_2, y_1, y_2):
        ...
        # p_s = numba_func(X_f)
        # q_s = numba_func(X_b)
        # X_f_T = np.ascontiguousarray(X[x_1:x_2, :].T)
        # X_b_T = np.ascontiguousarray(X[y_1:y_2, :].T)
        # X_f, X_b = gpuarray.to_gpu(X_f), gpuarray.to_gpu(X_b),
        # X_f_T, X_b_T = gpuarray.to_gpu(X_f_T), gpuarray.to_gpu(X_b_T)
        # pq = linalg.dot(X_f, X_b,transb="T")
        # # p_s = linalg.norm(X_f)
        # # q_s = linalg.norm(X_b)
        #
        # return (pq/(p_s+q_s-pq)).get()
